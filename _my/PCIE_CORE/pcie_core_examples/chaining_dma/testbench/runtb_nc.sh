...
QUARTUS_ROOTDIR='c:/altera/10.1/quartus'
...
then
   ...
   [ -d hardcopyiv_hssi     ] || mkdir hardcopyiv_hssi
   [ -d hardcopyiv_pcie_hip ] || mkdir hardcopyiv_pcie_hip
fi
...
# run simulation
ncelab -ACCESS +r pcie_core_chaining_testbench
ncsim -input runtb_nc.do pcie_core_chaining_testbench -LOGFILE transcript
